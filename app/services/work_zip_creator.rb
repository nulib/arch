class WorkZipCreator
  ...
  self.working_dir = Pathname.new(Dir.tmpdir) + 'nulib_zip_create'
  ...
  def create_zip(filepath: nil, callback: nil)
    ...
    count = work_presenter.member_presenters.size
    ...
    Zip::File.open(filepath, Zip::File::CREATE) do |zipfile|
      ...
      work_presenter.file_set_presenters.each_with_index do |file_set_presenter, index|
        file_set = FileSet.find(file_set_presenter.id)
        filename = "#{format '%03d', index + 1}-#{work_id}-#{file_set.id}-#{file_set.label}.jpg"
        ...
        if callback && ((index % 3).zero? || index == count - 1)
  ...
  protected
    ...
    def work_presenter
      @work_presenter ||= begin
        ...
        Hyrax::WorkShowPresenter.new(solr_doc, Ability.new(nil))
      end
    end
